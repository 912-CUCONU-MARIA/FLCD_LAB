...
public class SymbolTableTest {
    ...
    public static void testSymbolTable(){
        try {
            ...
            addIntConstant(2);
            assertPosition(2, symbolTable.getPositionIntConstant(2), new Pair<>(2, 0));

            addIntConstant(3);
            assertPosition(3, symbolTable.getPositionIntConstant(3), new Pair<>(3, 0));

            addIntConstant(104);
            assertPosition(104, symbolTable.getPositionIntConstant(104), new Pair<>(3, 1));

            addIntConstant(401);
            assertPosition(401, symbolTable.getPositionIntConstant(401), new Pair<>(98, 0));

            addStringConstant("ana");
            assertPosition("ana", symbolTable.getPositionStringConstant("ana"), new Pair<>(1, 0));

            addStringConstant("are");
            assertPosition("are", symbolTable.getPositionStringConstant("are"), new Pair<>(9, 0));

            addStringConstant("naa");
            assertPosition("naa", symbolTable.getPositionStringConstant("naa"), new Pair<>(1, 1));


            // Additional assertions for non-present keys
            assertPosition("aaaa", symbolTable.getPositionIdentifier("aaaa"), new Pair<>(-1, -1));
            assertPosition(22, symbolTable.getPositionIntConstant(22), new Pair<>(-1, -1));
    ...
    private static void addIntConstant(int value) throws Exception {
        System.out.println(value + " -> " + symbolTable.addIntConstant(value));
    }

    private static void addStringConstant(String value) throws Exception {
        System.out.println(value + " -> " + symbolTable.addStringConstant(value));
    }
    ...
    private static void assertContainsIntConstant(int value, boolean expected) {
        try {
            boolean result = symbolTable.containsIntConstant(value);
    ...
    private static void assertContainsStringConstant(String value, boolean expected) {
        try {
            boolean result = symbolTable.containsStringConstant(value);
